...
func (dec *Decoder) decodeWithOptionBorsh(v interface{}, opt option) (err error) {
...
func (dec *Decoder) decodeBorsh(rv reflect.Value, opt option) (err error) {
	...
	// Reset optionality so it won't propagate to child types. opt is a value
	// copy so we mutate it locally without affecting the caller.
	opt.is_OptionalField = false
	opt.is_COptionalField = false
	...
	case reflect.Slice:
		...
		default:
			// Pre-size and decode in place; avoids per-element reflect.New
			// and the O(log n) reallocs that reflect.Append would trigger.
			slc := reflect.MakeSlice(rt, l, l)
			elOpt := option{Order: opt.Order}
			for i := range l {
				if err = dec.decodeBorsh(slc.Index(i).Addr(), elOpt); err != nil {
					return
				}
			}
			rv.Set(slc)
	...
	case reflect.Map:
		...
		for i := 0; i < int(l); i++ {
			...
			err := dec.decodeBorsh(key.Elem(), mapOpt)
			if err != nil {
				return err
			}
			val := reflect.New(rt.Elem())
			err = dec.decodeBorsh(val.Elem(), mapOpt)
			if err != nil {
				return err
			}
...
func (dec *Decoder) deserializeComplexEnum(rv reflect.Value) error {
	...
	return dec.decodeBorsh(field, defaultOption)
}
...
func (dec *Decoder) decodeStructBorsh(rt reflect.Type, rv reflect.Value) (err error) {
	...
	if plan.isComplexEnum {
		return dec.deserializeComplexEnum(rv)
	}

	// sizes is non-nil only when the struct actually has sizeof wiring;
	// allocated on the stack via sizesScratch for small structs.
	var sizes []int
	if plan.hasSizeOf {
	...
	for i := range plan.fields {
		...
		// Fast primitive path: no option construction, no kind switch, just an
		// inline read straight into the field's memory.
		if fp.borshFastDecode != nil {
			if err = fp.borshFastDecode(dec, rv.Field(i)); err != nil {
		...
		opt := option{
			...
			is_COptionalField: fp.tag.COption,
			Order:             fp.tag.Order,
		}
		...
		if fp.ptrImplementsUnmarshaler || fp.valImplementsUnmarshaler {
			ft := fp.fieldType
			switch {
			case fp.ptrImplementsUnmarshaler:
				m := reflect.New(ft)
				val := m.Interface()
			...
			case fp.valImplementsUnmarshaler:
				m := reflect.New(ft.Elem())
				val := m.Interface()
				if err := val.(BinaryUnmarshaler).UnmarshalWithDecoder(dec); err != nil {
					return err
				}
			...
			if err = dec.decodeBorsh(v, opt); err != nil {
				return fmt.Errorf("error while decoding %q field: %w", fp.name, err)
			}
		}
