...
func (e *Encoder) encodePrimitive(rv reflect.Value) (isPrimitive bool, err error) {
	isPrimitive = true
	switch rv.Kind() {
	// case reflect.Int:
...
func (e *Encoder) encodeBorsh(rv reflect.Value, opt option) (err error) {
	...
	// copy so we mutate locally without affecting the caller.
	opt.is_OptionalField = false
	opt.is_COptionalField = false
	...
	isPrimitive, err := e.encodePrimitive(rv)
	if isPrimitive {
		return err
	}
...
func (enc *Encoder) encodeComplexEnumBorsh(rv reflect.Value) error {
	...
	// Encode the value if it's a primitive type
	isPrimitive, err := enc.encodePrimitive(field)
	if isPrimitive {
		return err
	}
	return nil
...
func (e *Encoder) encodeStructBorsh(rt reflect.Type, rv reflect.Value) (err error) {
	...
	// The fast primitive encode closures use unsafe.Pointer(fv.UnsafeAddr())
	// which requires the struct to be addressable. When the caller passed the
	// struct by value (e.g. enc.Encode(myStruct)) we fall back to the generic
	...
	for i := range plan.fields {
		...
		if fastOK && fp.borshFastEncode != nil {
			if err := fp.borshFastEncode(e, rv.Field(i)); err != nil {
		...
		if err := e.encodeBorsh(fv, opt); err != nil {
			return fmt.Errorf("error while encoding %q field: %w", fp.name, err)
		}
	}
	return nil
