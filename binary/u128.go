...
// Bytes returns the 16-byte big-endian representation of the value, regardless
// of the configured Endianness. The big-endian form is what (*big.Int).SetBytes
// expects and is what BigInt/String/DecimalString consume downstream — this
// function exists to feed those, not to produce wire bytes. For wire encoding,
// use Encoder.WriteUint128 with the desired byte order.
...
func (i *Uint128) unmarshalJSON_decimal(s string) error {
	...
	// FillBytes panics on negatives or values that don't fit — guard both.
	if parsed.Sign() < 0 {
		return fmt.Errorf("uint128: negative value %q", s)
	}
	if parsed.BitLen() > 128 {
		return fmt.Errorf("uint128: value %q exceeds 128 bits", s)
	}
...
func (i *Uint128) unmarshalJSON_hex(s string) error {
	...
	if len(truncatedVal) != 32 {
		return fmt.Errorf("uint128 expects 32 hex characters after 0x, had %d", len(truncatedVal))
	}
...
func (i *Uint128) UnmarshalWithDecoder(dec *Decoder) error {
	order := i.getByteOrder()
	if dec.currentFieldOpt.Order != nil {
...
func (i Uint128) MarshalWithEncoder(enc *Encoder) error {
	order := i.getByteOrder()
	if enc.currentFieldOpt.Order != nil {
